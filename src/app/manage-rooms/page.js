...
const ManageRoomsPage = () => {
  ...
  const ensureAdminAuth = () => {
    if (sessionStorage.getItem('adminAuthorized') === '1' || adminAuthorized) return true;
    setAdminPwdModal(true); return false;
  };

  const handleAdminPwd = (pwd) => {
    if (pwd === ADMIN_PWD) { 
      setAdminAuthorized(true); 
      try{sessionStorage.setItem('adminAuthorized','1');}catch(e){} 
      if (pendingAdminAction) pendingAdminAction(); 
    }
    else { alert('Falsches Passwort'); }
    setAdminPwdModal(false); setPendingAdminAction(null);
  };
  ...
  return (
    <>
      ...
      <PasswordModal open={adminPwdModal} title="Admin Passwort" message="Bitte Admin Passwort eingeben" onSubmit={handleAdminPwd} onCancel={()=>{ setAdminPwdModal(false); }} />
    </>
  );
};
